...
export async function POST(request: Request) {
  ...
  const { type, beskrivelse, tittel, sted, tid, status } = body;
  const filter = { beskrivelse: beskrivelse, tittel: tittel, tid: tid };

  const update = { type: type, beskrivelse: beskrivelse, tittel: tittel, sted: sted, tid: tid, status: status };
